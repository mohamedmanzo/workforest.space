...
- Can't delete a branch that's checked out in a worktree
- Worktrees share the same remote configuration
...
cd ..  # parent repo
git add myproject-feature
git commit -m "Update feature progress"
```

### Initialization

When cloning the parent repo:
...
```bash
git submodule foreach 'git fetch origin && git rebase origin/main || true'
```
...
rm -rf .git/modules/myproject-feature
